...
def main():
    ...
    # Initialize polarization controller in simulation mode with queues
    pol_controller = PolarizationController(
        physical=False,
        pulses_queue=pulses_queue,
        polarized_pulses_queue=polarized_pulses_queue,
        laser_info=laser_info
    )
    pol_controller.initialize()
    ...
    while not polarized_pulses_queue.empty():
        ...
        logger.info(f"  Polarized pulse: {polarized_pulse.photons} photons, "
                   f"basis={polarized_pulse.basis}, bit={polarized_pulse.bit_value}, "
                   f"angle={polarized_pulse.polarization_angle}°")
    ...
    for basis, bit, description in test_states:
        ...
        if not polarized_pulses_queue.empty():
            ...
            logger.info(f"    Result: basis={result_pulse.basis}, bit={result_pulse.bit_value}, "
                       f"angle={result_pulse.polarization_angle}°")
