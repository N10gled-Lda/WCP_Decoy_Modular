...
key_lengths = [1000]
key_lengths = [100, 1000, 2000, 5000, 10000]
...
# plt.grid(True)
# plt.show()
# plt.savefig("BB84/tests/bandwidth/images/alice_payload_sizes_vs_bytes_per_frame.png")

plt.figure(figsize=(GOLDEN_RATIO * 6, 6))
# plt.plot(key_lengths, [bw * 10**-3 for bw in bandwidth], marker='o', label="Bandwidth Downlink")
plt.plot(key_lengths, [bw for bw in bandwidth_kbps], marker='o', label="Bandwidth Downlink")
...
plt.legend()
plt.tight_layout()
plt.savefig("BB84/tests/bandwidth/images/bandwidth_downlink_vs_key_length.png")
plt.show()

# Save data to file
with open("BB84/tests/bandwidth/data/alice_payload_sizes_vs_bytes_per_frame.csv", "w") as f:
...
with open("BB84/tests/bandwidth/data/bandwidth_downlink_vs_key_length.csv", "w") as f:
