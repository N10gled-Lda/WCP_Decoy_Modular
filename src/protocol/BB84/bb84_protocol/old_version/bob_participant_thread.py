...
class Bob(BB84ProtocolSimulationParticipant):
    ...
    def __init__(self, num_qubits: int = 100, qubit_delay_us: int = 1000, num_frames: int = 10, bytes_per_frame: int = 10, sync_interval: float = 1, sync_frames: int = 100, sync_bytes_per_frame: int = 50, loss_rate: float = 0.9, test_bool: bool = True, test_fraction: float = 0.1, error_threshold: float = 0.0, thread_id_byte: bytes = None, start_idx:int = 0, send_deque: deque = None, lock: th.Lock = None):
        ...
        self.size_thread_id_bytes = len(thread_id_byte)
    ...
    def setup_client(host: str, port: int) -> socket.socket:
        ...
        return client
    def receive_with_thread_id(self, connection: socket.socket) -> Tuple[int, bytes]:
        """Receive data with thread ID."""
        data = connection.recv(self.size_thread_id_bytes + 1) # Receive thread ID and data
        thread_id_b = data[:self.size_thread_id_bytes]
        data = data[self.size_thread_id_bytes:]
        return thread_id_b, data
    ...
    def send_detected_idx(self, connection: socket.socket):
        ...
        # self.classical_channel.send_data_th(self.detected_idxs, self.thread_id_byte, connection)
        data_to_send = (self.thread_id, self.detected_idxs)
        with self._socket_lock:
            self.send_deque.appendleft(data_to_send)
            print(f"Bob: Added detected idx of thread {self.thread_id} to send deque.")
            # print(f"Bob: Send deque size({len(self.send_deque)}): {self.send_deque}")
    ...
    def receive_detected_bases(self, connection: socket.socket):
        ...
        while not self.receive_deque:
            pass
        self.other_detected_bases = self.receive_deque.pop()
    ...
    def send_common_indices(self, connection: socket.socket):
        ...
        data_to_send = (self.thread_id, self.common_indices)
        with self._socket_lock:
            self.send_deque.appendleft(data_to_send)
            print(f"Bob: Added common_indices of thread {self.thread_id} to send deque.")
            # print(f"Bob: Send deque size({len(self.send_deque)}): {self.send_deque}")
    ...
    def send_test_bits(self, connection: socket.socket):
        ...
        data_to_send = (self.thread_id, [self.common_test_indices, self.common_test_bits])
        with self._socket_lock:
            self.send_deque.appendleft(data_to_send)
            print(f"Bob: Added test bits of thread {self.thread_id} to send deque.")
    ...
    def receive_test_result(self, connection: socket.socket):
        ...
        while not self.receive_deque:
            pass
        [self.test_success_bool, self.failed_percentage] = self.receive_deque.pop()
