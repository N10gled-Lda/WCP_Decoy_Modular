...
class BobQubits():
                ...
                while True:
                    ...
                    if byte == self._start_marker:
                        ...
                        if start_time is not None:
                            bytes_placeholder_list.append(byte)
    ...
    def run_mock_bob_qubits(self, connection: socket.socket, wait_time: bool = True, rand_fake_error_bool: bool = False, rand_fake_error_rate: float = 0.0, fixed_fake_error_bool: bool = False, fixed_fake_error_rate: float = 0.0, lock: threading.Lock = None):
        """Run Bob's part for a single thread to obtain part of the final key."""
        self.recv_lock = lock
        try:
            #self.receive_sync(connection)
    ...
    def run_mock_bob_qubits_queue(self, shared_queue: queue.Queue, connection: socket.socket, wait_time: bool = True, rand_fake_error_bool: bool = False, rand_fake_error_rate: float = 0.0, fixed_fake_error_bool: bool = False, fixed_fake_error_rate: float = 0.0):
        """Run Bob's part for a single thread to obtain part of the final key."""
        logger.info("Receiving qubits from Alice...")
        try:
    ...
    def receive_qubits_mock_queue(self, shared_queue: queue.Queue, wait_time: bool = True):
        ...
        try:
            ...
            [self.detected_qubits_bytes, self.average_time_bin] = data
    ...
    def process_data_mock_queue(self, rand_fake_error_bool: bool = False, rand_fake_error_rate: float = 0.0, fixed_fake_error_bool: bool = False, fixed_fake_error_rate: float = 0.0):
        logger.info("Transmission complete. Transform time of detections to indices.")
        print(f"Average time bin: {self.average_time_bin}")
        # Simulate the fake error, if rand_fake_error_bool is True, change the detected qubits
        if rand_fake_error_bool and 0 < rand_fake_error_rate < 1:
            num_error = int(rand_fake_error_rate * len(self.detected_qubits_bytes))
            ...
            logger.info(f"Simulated random error rate: {100*rand_fake_error_rate:.1f}%")
        ...
        # Simulate a fixed error rate, if fixed_fake_error_bool is True, change the detected bits
        if fixed_fake_error_bool and 0 < fixed_fake_error_rate < 1:
            num_error = int(fixed_fake_error_rate * len(self.detected_bits))
            error_indices = random.sample(range(len(self.detected_bits)), num_error)
            for i in error_indices: # Switch the 0/1 bit
                if self.detected_bits[i] == 0:
                    self.detected_bits[i] = 1
                elif self.detected_bits[i] == 1:
                    self.detected_bits[i] = 0
                else:
                    logger.error(f"Detected bit is not binary: {self.detected_bits[i]}")
            logger.info(f"Simulated fixed error rate: {100*fixed_fake_error_rate:.1f}%")
    ...
    def receive_qubits_mock(self, connection: socket.socket, wait_time: bool = True, rand_fake_error_bool: bool = False, rand_fake_error_rate: float = 0.0, fixed_fake_error_bool: bool = False, fixed_fake_error_rate: float = 0.0):
        """Simulates receiving qubits with detection times with some loss."""
        logger.info("Receiving qubits from Alice...")
        ...
        try:
            ...
            # if start != self._start_marker.to_bytes(1, 'big'):
            #     logger.warning("Start marker not received properly")
            # else:
            #     received_data = b""
            #     while True:
            ...
            #         if self._end_marker.to_bytes(1, 'big') in chunk:
            #             # Split off payload and ignore anything after the end marker
            #             payload, _ = chunk.split(self._end_marker.to_bytes(1, 'big'), 1)
            ...
            #         logger.error("Deserialization failed: " + str(e))
            ...
            while True:
                ...
                if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                    ...
                    try:
                        self.detected_qubits_bytes, self.average_time_bin = pickle.loads(payload)
                        logger.info(f"Received {len(self.detected_qubits_bytes)} qubits from Alice without loss")
            ...
            # # Receive data until we find the start marker
            ...
            # # Extract data after the start marker
            # _, buffer = buffer.split(START_MARKER, 1)

            # # Continue receiving until we find the end marker
            # received_data = b''
            ...
            # # Get the data between markers
            # data_part, buffer = buffer.split(END_MARKER, 1)
            ...
            logger.error(f"Error in receiving qubits: {e}")
            print("Error in receiving qubits:")
...
class BobThread():
    ...
    def send_data(self, data: any, list_size_to_append: list = None):
        ...
        #self.messages_sent.append(("B", "S", self.number_messages_sent, time.process_time(), len(payload), 1, self._thread_id))
        self.messages_sent.append(
            ("B", "S", self.number_messages_sent, time.perf_counter(), len(payload), 1, self._thread_id))
    ...
    def receive_detected_bases(self):
        ...
        payload_received = pickle.dumps((self._thread_id, self._other_detected_bases))
        #self.messages_received.append(("B", "R", self.number_messages_received, time.process_time(), len(payload_received), 4, self._thread_id))
        self.messages_received.append(
            ("B", "R", self.number_messages_received, time.perf_counter(), len(payload_received), 4, self._thread_id))
    ...
    def receive_test_result(self):
        ...
        payload = pickle.dumps((self._thread_id, [self.test_success_bool, self.failed_percentage]))
        #self.messages_received.append(("B", "R", self.number_messages_received, time.process_time(), len(payload), 4, self._thread_id))
        self.messages_received.append(
            ("B", "R", self.number_messages_received, time.perf_counter(), len(payload), 4, self._thread_id))
        self.number_messages_received += 1
