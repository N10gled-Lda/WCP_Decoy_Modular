...
def test_get_and_set_correct_parity_pf():
    Key.set_random_seed(6663)
    Shuffle.set_random_seed(6664)
    key_size = 1
    ...
    shuffle = Shuffle(key.get_size(), Shuffle.SHUFFLE_RANDOM)
    #print(shuffle.__str__())
    #assert shuffle.__str__() == "0->3 1->5 2->7 3->8 4->0 5->2 6->1 7->4 8->6 9->9"
    block_size = 1
    ...
    block_1 = blocks[0]
    #print(block.__str__())
    #assert block.__str__() == "011011011"
    #assert block.get_correct_parity() is None
    #block.set_correct_parity(0)
    #assert block.get_correct_parity() == 0
    #block.set_correct_parity(1)
    ...
    aux_size_single_block = pickle.dumps(block)
    aux_size_single_block_1 = pickle.dumps(block_1)
    aux_size_all_blocks = pickle.dumps(blocks)

    print(f"Key Size: {key_size}")
    print(f"Block Size: {block_size}")
    print(f"Number of Blocks: {len(blocks)}")
    print(f"A single Block has a size of: {len(aux_size_single_block)} Bytes")
    print(f"A single Block 1 has a size of: {len(aux_size_single_block_1)} Bytes")
    print(f"All Blocks total a size of: {len(aux_size_all_blocks)}")
...
if __name__ == "__main__":
    test_get_and_set_correct_parity_pf()
