...
# from study.alice_sim import ask_parities_secure_optimized
...
class Reconciliation:
    ...
    def __init__(self, algorithm_name, classical_channel, noisy_key, estimated_bit_error_rate, aux_socket=None,
                 number_r=0, number_runs=0, it_number=0, it_n_total=0, median_socket=None, thread_id=None,
                 send_buffer=None, lock=None, participant=None):
        """
    ...
    def reconcile(self):
        ...
        # AQUI PODERA ESTAR JA A RESPOSTA A PERGUNTA DO RUI DE QUANTOS BITS SAO PASSADOS
    ...
    def end_communication_channel(self):
        ...
        # self.connection.sendall(init_pack)
        # self.connection.sendall(pickle.dumps("End_reconciliation_final"))
        # self.connection.sendall(end_pack)
    ...
    def end_communication_channel_threading(self):
        with self.lock:
            print(f"Send Buffer Thread Id: {self.thread_id} | End Reconciliation FINAL")
            self.send_buffer.appendleft((self.thread_id, "end_reconciliation_final"))
    ...
    def end_reconcile(self):
        ...
        # self.connection.sendall(init_pack)
        # self.connection.sendall(pickle.dumps("End_reconciliation"))
        # self.connection.sendall(end_pack)
        self.aux_socket.sendall(init_pack)
    ...
    def end_reconcile_threading(self):
        with self.lock:
            print(f"Send Buffer Thread Id: {self.thread_id} | End Reconciliation")
            self.send_buffer.appendleft((self.thread_id, "end_reconciliation"))

    def reconcile_channel(self, propagation_delay=GEO_DELAY, bandwidth=-1):
        """
        Run the Cascade algorithm to reconciliate our ("Bob's") noisy key with the server's
        ("Alice's") correct key.
        ...
        """
        print("Waiting for Alice to be ready to connect")
        # Connect to Alice for data transmission
        #self.my_socket = socket.create_connection((self.alice_ip_address, self.alice_port), timeout=10)
        try:
            self.my_socket.listen(1)

            self.connection, addr = self.my_socket.accept()

            #self.aux_socket.listen(10)
            #self.connection, addr = self.aux_socket.accept()

        except Exception as e:
            print(f"Alice: Error occurred 2 - {e}")
        ...
        # if self.number_r == (self.number_runs - 1):

        # CHECK HERE

        """if self.it_number == self.it_n_total:
            init_pack = b"begin_ask"
            ...
            self.connection.sendall(init_pack)
            self.connection.sendall(pickle.dumps("End_reconciliation2"))
            self.connection.sendall(end_pack)
        else:"""
        ...
        # This is made since n_chunks_per_block is a sum of all the chunks of each block (when divided by n_blocks will give average chunks)
        # self.stats.n_chunks_per_block /= self.stats.ask_parity_blocks
        # self.stats.n_chunks_per_block /= self.stats.ask_parity_messages
    ...
    def reconcile_channel_threading(self, propagation_delay=GEO_DELAY, bandwidth=-1):
        """
        Run the Cascade algorithm to reconciliate our ("Bob's") noisy key with the server's
        ("Alice's") correct key.
    ...
    def _schedule_ask_correct_parity_threading(self, block, correct_right_sibling):

        self._save_block_threading(block, correct_right_sibling)

        block_start_index = block.get_start_index()
        block_end_index = block.get_end_index()
        block_shuffle_index_to_key_index = block.get_shuffle().get_shuffle_index_to_key_index()

        block_to_send = (self.thread_id, block_start_index, block_end_index, block_shuffle_index_to_key_index)

        # Adding the data to the block in order for the thread to register and eventually send to Alice
        self.send_buffer.appendleft(block_to_send)
        self.counter_ops += 1
        print(
            f"--Data added to Send Buffer Thread Id: {self.thread_id} | Block To Send: Start Index: {block_start_index} End Index: {block_end_index}")

    def _schedule_ask_correct_parity_threading_v2(self, block, correct_right_sibling):
        # Adding an item to the end (not the start!) of a list is an efficient O(1) operation.
        entry = (block, correct_right_sibling)
        self._pending_ask_correct_parity.append(entry)
    ...
    @staticmethod
    def _produce_key_string(start_index, end_index):
        return f"{start_index}-{end_index}"

    def _save_block_threading(self, block, correct_right_sibling):
        start_index = block.get_start_index()
        end_index = block.get_end_index()

        key_str = self._produce_key_string(start_index, end_index)

        self.block_dict[key_str] = (block, correct_right_sibling)
        self.block_dict_aux[key_str] = (block, correct_right_sibling)

    def _have_pending_ask_correct_parity(self):
        return self._pending_ask_correct_parity != []

    def _have_pending_sending_ask_correct_threading(self):
    ...
    @staticmethod
    def _bits_in_int_multiple(int_value_a, int_value_b, int_value_c):
    ...
    def add_receiving_information(self, start_end_block):
    ...
    # This method should be processed via threading to keep on listening on the receiving buffer and then add the information to the buffer
    def _service_processing_receive_message_from_alice_threading(self):
        # TODO: It will listen indefinetly and when a message is received, it will then process the data

        while True:

            # Listening for message and reply of the parity bits
            data, aux = self._receive_message_from_alice()

            # TODO: Process data and insert in self.receive_buffer deque using appendleft
            self.stats.reply_parity_bits += len(data) * 8
            if (len(data) * 8) > self.largest_message_received[0]:
                self.largest_message_received = (len(data) * 8, self.stats.ask_parity_messages)

            reply_correct_parity_block = pickle.loads(data)

            start_index, end_index, correct_parity = reply_correct_parity_block

        pass

    # This method should be run asynchronously
    # ASYNC
    def process_parity_messages(self):
        while self.counter_ops > 0:
            while self.receive_buffer:
                start_index, end_index, correct_parity = self.receive_buffer.pop()

                key_str = self._produce_key_string(start_index, end_index)

                block, correct_right_sibling = self.block_dict[key_str]

                # Set the correct parity for the block
                block.set_correct_parity(correct_parity)

                # self._schedule_try_correct(block, correct_right_sibling)
                # Tries to Correct the Block
                # Correct_right_sibling should be placed in the block_dict
                # self.errors_corrected += self._try_correct_threading(block, correct_right_sibling, True)

                self._schedule_try_correct(block, correct_right_sibling)

                self.counter_ops -= 1

                self.block_dict_aux.pop(key_str)

                print(f"Thread Id: {self.thread_id} | Errors Corrected: {self.errors_corrected}")

            self._service_pending_try_correct(True)

            print(f"length the aux: {len(self.block_dict_aux)}")
            print(f"Counter ops: {self.counter_ops}")
    ...
    def _receive_message_from_alice(self):
        ...
        # if self.bandwidth > 0:
        #    time_aux = len(data_mss)/self.bandwidth
        # else:
        #    time_aux = 0
    ...
    @staticmethod
    def _simulate_time_passing(sleep_time):
        start_time = time.perf_counter()
    ...
    def _service_pending_ask_correct_parity_channel(self):
        ...
        # ATTENTION
        # This array is created to fix the issue of sending the entire Block object, as the Block object contains the noisy key leading to a very unsecure process
        ask_parity_blocks_secure = []
        ...
        for entry in self._pending_ask_correct_parity:
            ...
            block_secure_obj = (block_start_index, block_end_index, block_shuffle_object)

            ask_parity_blocks_secure.append(block_secure_obj)
            ...
            # This is made to obtain a more correct count of the number of chunks for each block
            # aux_block_data = pickle.dumps(block)
            # n_chunks_of_block = len(aux_block_data) / 4096
            # self.stats.n_chunks_per_block += n_chunks_of_block
        ...
        if __debug__:
            ...
            print(f"Number of Parity Blocks (Len(Parity Blocks)): {len(ask_parity_blocks)}")
            print(f"N Chunks Per Block: {(len(package_data) / 4096) / len(ask_parity_blocks)}")
        ...
        if __debug__:
            print(f"time to receive Reply: {to_tim - GEO_DELAY}")
        ...
        assert len(correct_parities) == len(ask_parity_blocks_secure_optimized)
    ...
    def _service_pending_ask_correct_parity_channel_threading(self):
        ...
        print(f"Receive Buffer Length: {len(self.receive_buffer)}")
        ...
        # Cycle for to wait to receive the message
        # while self.counter_ops:
        ...
        correct_parities = message_received
        ...
        # self.stats.reply_parity_bytes += len(package_received)
        package_received = pickle.dumps(message_received)
        self.stats.reply_parity_bytes += len(package_received)
    ...
    def _service_pending_ask_correct_parity_channel_threading_v2(self):
    ...
    '''
    #This method corresponds to the process of sending messages and should be implemented via Threading
    def _service_pending_ask_correct_parity_channel_threading(self):


        #Infinite Cycle - Since the thread will constantly be running to process sending messages ansychronously
        while True:
            #If buffer has elements
            if self.send_buffer:
                #TODO: Implementation of the functionality - Pop element and process sending of the message

                ask_parity_block = self.send_buffer.pop()

                self.stats.ask_parity_messages += 1
                self.stats.ask_parity_blocks += 1

                # Due to optimised version to not send any Objects
                # package_data = pickle.dumps(ask_parity_block)
                package_data = pickle.dumps(ask_parity_block)
                ...
                print(f"Thread Sending - Sent Ask Parity Block Message")
    '''
    ...
    def _one_normal_cascade_iteration_channel(self, iteration_nr):
    ...
    def _one_normal_cascade_iteration_channel_threading(self, iteration_nr):
    ...
    # Small auxiliar function to split a list in two
    @staticmethod
    def split_list(a_list):
        half = len(a_list) // 2
        return a_list[:half], a_list[half:]

    # Auxiliar method in order to verify that each block is rightly identified (we can have blocks with same parity but have errors)
    @staticmethod
    def check_difference_keys(noisy_key, correct_key, start_index):
        correct = True
        for index, bit in enumerate(noisy_key):
            if bit != correct_key.get_bit(index + start_index):
                correct = False
    ...
    def _one_normal_cascade_iteration_minimum(self, iteration_nr):
        # This implementation should be considered for the minimization of ask_parity_messages, Regarding the division of key in blocks for the first iteration,
        # instead of asking for all the parity for all the blocks. Bob simply sends the shuffle (1st iteration is equal), and sends the block division. Alice receiving the message
        # will then send the parity for each of the block divided. This will produce a decrease in number of bits of "ask_parity_message". However this approach may need additional attention
        # detail when using Parallel computing. This approach simply means that the number of bits Bob asks Alice initially is drastically reduced. Bob will still need to ask for specific blocks when parities are "ODD"
        ...
        # select random seed
        if iteration_nr == 1:
            # the first iteration keep the key unshuffled
            shuffle_seed = 0

            # This should be changed for efficiency in case it is the final correct approach
            shuffle_minimum = Shuffle(self._reconciled_key.get_size(), Shuffle.SHUFFLE_KEEP_SAME)
        else:
            # Generate a random seed to obtain the same results for the shuffling: Theoretically this seed can have any number of bits desired (This represents the way a shuffling it is represented when communicating through the classical channel)
            shuffle_seed = random.randint(1, 100)
            shuffle_minimum = Shuffle(self._reconciled_key.get_size(), Shuffle.SHUFFLE_RANDOM,
                                      shuffle_seed=shuffle_seed)

        # This represents the initiation of communication - sending the block size (ie the number of divisions for the block) (8 bits) - sending the shuffling seed (7 bits) -
        # - sending the iteration number (2 bit) which might be necessary for receiving the messages
        # self.stats.start_iterations_bits += 2 + 8 + 7
        self.stats.start_iterations_bits += 2 + 8 + max(shuffle_minimum.get_identifier().bit_length(), 1)

        alice_bits_parity_blocks, copy_corrected_key = self._classical_channel.start_reconciliation_minimum(
            iteration_nr, shuffle_seed, block_size, self)

        # Increase the asked Parity Blocks
        # This is commented because in the beggining of iteration Bob doesnt ask for specific blocks, only gives seed and division
        # Empirically this is simply Reply_blocks
        # self.stats.ask_parity_blocks += len(alice_bits_parity_blocks)

        self.aux_copy_correct_key = copy_corrected_key

        copy_reconciled = copy.deepcopy(self._reconciled_key)

        if shuffle_seed:
            random_generator = random.Random(shuffle_seed)
            random_generator.shuffle(copy_reconciled._bits)
            # random_generator.shuffle(self._reconciled_key._bits)

        bob_bits_parity_blocks = []

        # Copied from mock_classical_channel

        if __debug__:
            # print(f"Block Size: {block_size}", file=self.file_logging)
            # print("Bob Blocks:              ", end=" ", file=self.file_logging)
            self.print_message(f"Block Size: {block_size}")
            self.print_message("Bob Blocks:              ", end=" ")

            add_start_string = " " * (iteration_nr - 1)
            add_end_string = " " * (iteration_nr - 1)

        check_differences = []

        for i in range(0, copy_reconciled.get_size(), block_size):
            aux_block = []
            for index in range(i, i + block_size):
                if index < copy_reconciled.get_size():
                    aux_bit = copy_reconciled.get_bit(index)
                    # aux_bit = self._reconciled_key.get_bit(index)
                    aux_block.append(aux_bit)

            if __debug__:
                # print(self._format_key_array(aux_block, copy_corrected_key, i), end=add_start_string+" | " + add_end_string, file=self.file_logging)
                self.print_message(self._format_key_array(aux_block, copy_corrected_key, i),
                                   end=add_start_string + " | " + add_end_string)
            check_differences.append(self.check_difference_keys(aux_block, copy_corrected_key, i))
            parity_of_block = self.calculate_parity_sub_block(aux_block)
            bob_bits_parity_blocks.append(parity_of_block)

        if __debug__:
            # print(f"\nBob Block Parity Bits:   {bob_bits_parity_blocks}", file=self.file_logging)
            self.print_message(f"\nBob Block Parity Bits:   {bob_bits_parity_blocks}")

        # Improve later

        # alice_bits_parity_blocks = self._classical_channel.start_reconciliation_minimum(iteration_nr, shuffle_seed, block_size, self.file_logging)

        # This is essentially a XOR that will tell which blocks might have the errors
        # Might be suitable create an yield function
        difference_in_bits = [el1 ^ el2 for el1, el2 in zip(bob_bits_parity_blocks, alice_bits_parity_blocks)]

        if __debug__:
            # print(f"Difference in Bits:      {difference_in_bits}", file=self.file_logging)
            self.print_message(f"Difference in Bits:      {difference_in_bits}")
            for bit, correct in zip(difference_in_bits, check_differences):
                if bit:
                    # print(f"{YELLOW}{bit}{RESET}", end=" ",file=self.file_logging)
                    self.print_message(f"{YELLOW}{bit}{RESET}", end=" ")
                elif correct:
                    # print(f"{GREEN}{bit}{RESET}", end=" ",file=self.file_logging)
                    self.print_message(f"{GREEN}{bit}{RESET}", end=" ")
                else:
                    # print(f"{RED}{bit}{RESET}", end=" ", file=self.file_logging)
                    self.print_message(f"{RED}{bit}{RESET}", end=" ")
            # print("\n",file=self.file_logging)
            self.print_message("\n")

        for i, bit in enumerate(difference_in_bits):
            # do Cascade and ask for parity bits | i represents the block number (so to access block use block nr * blocksize?)
            # Only schedule blocks where the parities are ODD between Alice and Bob
            # start_index = i*block_size
            # end_index = start_index + block_size
            # parent_block = Block(self._reconciled_key, shuffle_minimum, start_index, end_index, None,
            #                    current_parity=bob_bits_parity_blocks[i], correct_parity=alice_bits_parity_blocks[i])
            # self._register_block_key_indexes(parent_block)

            # Cascade small block
            start_index = i * block_size
            # Min is calculated to prevent a block index created has more than the key size
            end_index = min(start_index + block_size, self._reconciled_key.get_size())

            # Parent Block for asking the parity bits in sub_blocks
            # parent_block = Block(self._reconciled_key, shuffle_minimum, start_index, end_index, None,
            #                     current_parity=bob_bits_parity_blocks[i], correct_parity=alice_bits_parity_blocks[i])
            parent_block = Block(self._reconciled_key, shuffle_minimum, start_index, end_index, None,
                                 current_parity=bob_bits_parity_blocks[i], correct_parity=alice_bits_parity_blocks[i])
            ...
            if bit:
                left_sub_block = parent_block.create_left_sub_block()
                right_sub_block = parent_block.create_right_sub_block()

                self._register_block_key_indexes(left_sub_block)
                self._register_block_key_indexes(right_sub_block)

                # I believe this is used only for the left_sub_block since after knowing left side and parent we can infer right side
                # Correct_right_sibling is set to true, because I believe if it is shown that the error is not on left side but on right side (since we can infer that knowing the left and parent), it will automatically correct right side
                self._schedule_ask_correct_parity(left_sub_block, True)

        errors_corrected = self._service_all_pending_work(True)
        self.stats.estimated_corrected_bits += errors_corrected
        self.stats.corrected_bits_error_iteration.append(errors_corrected)

    def _one_normal_cascade_iteration_minimum_channel(self, iteration_nr):
        # This implementation should be considered for the minimization of ask_parity_messages, Regarding the division of key in blocks for the first iteration,
        # instead of asking for all the parity for all the blocks. Bob simply sends the shuffle (1st iteration is equal), and sends the block division. Alice receiving the message
        # will then send the parity for each of the block divided. This will produce a decrease in number of bits of "ask_parity_message". However this approach may need additional attention
        # detail when using Parallel computing. This approach simply means that the number of bits Bob asks Alice initially is drastically reduced. Bob will still need to ask for specific blocks when parities are "ODD"
        ...
        packed_data = struct.pack("HII", iteration_nr, shuffle_seed, block_size)

        size_in_bytes = len(packed_data)

        size_in_bit = size_in_bytes * 8

        print(
            f"Start Iteration Bits - Previously: {self.stats.start_iterations_bits}\nPacked Data (Bytes): {size_in_bytes}\nPacked Data (Bits): {size_in_bit}")
        ...
        errors_corrected = self._service_all_pending_work(True)
        # print(f"Passed Service")
        self.stats.estimated_corrected_bits += errors_corrected
        self.stats.corrected_bits_error_iteration.append(errors_corrected)

    # Copied method from mock_classical_channel
    ...
    def _service_all_pending_work_channel(self, cascade):
    ...
    def _service_all_pending_work_channel_threading(self, cascade):
        ...
        while self._have_pending_try_correct() or self._have_pending_ask_correct_parity():
            ...
            self._service_pending_ask_correct_parity_channel_threading_v2()
    ...
    # This method should be run via Threading - To keep listening to the size of receive buffer and compute the parities or ask for the sibling parities
    def _try_correct_threading(self, block, correct_right_sibling, cascade):
        ...
        if not self._correct_parity_is_known_or_can_be_inferred(block):
            self._schedule_ask_correct_parity_threading(block, correct_right_sibling)
            return 0
        ...
        if block.get_error_parity() == Block.ERRORS_EVEN:
            if correct_right_sibling:
                return self._try_correct_right_sibling_block_threading(block, cascade)
            return 0
        ...
        if block.get_size() == 1:
            self._flip_key_bit_corresponding_to_single_bit_block_threading(block, cascade)
            return 1
        ...
        if left_sub_block is None:
            ...
            self._register_block_key_indexes(left_sub_block)
        return self._try_correct_threading(left_sub_block, True, cascade)

    def _try_correct(self, block, correct_right_sibling, cascade):

        # print(f"Try_correct Thread Id: {self.thread_id} | Counter: {self.counter_ops} Len Pending Try Correct: {len(self._pending_try_correct)}")
        # self.counter_ops += 1
        if __debug__:
            print(
            ...
            if parent_block is None:
                ...
                print(
                    f"Parent Block: {_format_block_key(parent_block, self._classical_channel._correct_key, parent_block.get_start_index(), parent_block.get_end_index())}",
                    file=self.file_logging)
    ...
    def _try_correct_right_sibling_block_threading(self, block, cascade):
        parent_block = block.get_parent_block()
        right_sibling_block = parent_block.get_right_sub_block()
        if right_sibling_block is None:
            right_sibling_block = parent_block.create_right_sub_block()
        ...
        return self._try_correct_threading(right_sibling_block, False, cascade)
    ...
    def _flip_key_bit_corresponding_to_single_bit_block_threading(self, block, cascade):
        ...
        for affected_block in self._get_blocks_containing_key_index(flipped_key_index):
            ...
            if cascade and affected_block.get_error_parity() != Block.ERRORS_EVEN:
                ...
                if self._algorithm.sub_block_reuse or affected_block.is_top_block():
                    ...
                    # Perhaps big modification in Logic, instead of SCHEDULING, IT TRIES TO CORRECT RIGHT AWAY
                    self._try_correct_threading(affected_block, False, True)
