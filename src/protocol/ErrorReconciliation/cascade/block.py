class Block:
    ...
    def __init__(self, key, shuffle, start_index, end_index, parent_block, current_parity=None, correct_parity=None):
        """
        ...
        #self._start_index = min(start_index, key.get_size() - 1)
        self._start_index = start_index
        #self._end_index = min(end_index, key.get_size() - 1)
        #self._end_index = min(end_index, key.get_size())
        ...
        #Parameters Current parity and Correct parity are only defined when using "minimum" functions in reconciliation.py

        # Calculate the current parity for this block.
        # Change by PF for better ability in minimum - Should be changed for a more consistent way
        if current_parity is None:
            self._current_parity = shuffle.calculate_parity(key, start_index, end_index)
        else:
            self._current_parity = current_parity
    ...
    def get_key_bits(self):
        ...
        for shuffle_index in range(self._start_index, self._end_index):
            key_index = self._shuffle.get_key_index(shuffle_index)
            key_bits.append(self._key.get_bit(key_index))
        return key_bits

    def get_key_bit(self, index):
        return self._key.get_bit(index)
