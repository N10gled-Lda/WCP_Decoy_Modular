...
"""
================================================================================
File Name: dwfconstants.py

Description: 
    Constants to be used with the Waveform SDK.
    Constants added by the user of Waveform SDK.

Author: Waveform SDK
================================================================================
"""
class DWFUser:
    """Defined variables for FDwfParamSet"""
    class TurnOffBehaviour(Enum):
        RUN      = c_int(0)
        STOP     = c_int(1)
        SHUTDOWN = c_int(2)

    """Defined variables for FDwfDeviceAutoConfigureSet"""
    class AutoConfigure(Enum):
        DISABLED = c_int(0)
        ENABLED  = c_int(1)
        DYNAMIC  = c_int(3)

    """Defined variables for FDwfAnalogOutConfigure"""
    class OutputBehaviour(Enum):
        STOP = c_int(0)
        START = c_int(1)
        APPLY = c_int(3)

    """Defined variables for FDwfDeviceOpen"""
    class Devices(Enum):
        FIRST = c_int(-1)

    """FDwfAnalogOutFunctionSet"""
    class AnalogOutFunctions(Enum):
        DC = c_ubyte(0)

    ALL_CHANNELS = c_int(-1)
###############################################################